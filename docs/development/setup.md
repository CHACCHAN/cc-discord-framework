...
- 公開パッケージのインストール時にはこの開発専用スクリプトを実行しません。
  ...
  壊さないためです。
- `node_modules` がまだ無ければ何もしません。既存のエントリは
  シンボリックリンクでもレジストリ版の実体でも取り除いて張り替えます —
  `bun install` は公開済みの `@cc-discord-framework/core` をこの位置に
  実体として展開するため、install のたびに `bun run link:self` の
  再実行が必要です。
