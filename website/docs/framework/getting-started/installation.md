...
- **discord.js 14** はフレームワークに同梱され、`cc-discord-framework`
...
このドキュメントが説明する **v2 は、まだ npm に公開されていません。**
npm にある `cc-discord-framework`(1.x 系)は旧世代のパッケージで、
このドキュメントの内容とは別物です。`bun add cc-discord-framework` で
入るのは 1.x なので注意してください。

v2 を今すぐ試す場合は GitHub リポジトリから追加します:

```sh
bun add github:CHACCHAN/cc-discord-framework
```

v2 のリリース後は、通常どおり `bun add cc-discord-framework` になります。
