...
公式プラグインはすべて npm の [`@cc-discord-framework`](https://www.npmjs.com/org/cc-discord-framework) スコープで公開されています。各ページの `bun add ...` がそのまま使えます。フレームワーク本体([`@cc-discord-framework/core`](https://www.npmjs.com/package/@cc-discord-framework/core))は peer dependency なので、プラグインを入れると一緒に要求されます。実際に動く構成例は、リポジトリの [`client/`](https://github.com/CHACCHAN/cc-discord-framework/tree/main/client) を参照してください。
